...
struct ChartYAxisLabelModifier<R: RootRegistry>: ViewModifier, Decodable {
    ...
    func body(content: Content) -> some View {
        if let title {
            ...
            case (nil, nil):
                content.chartYAxisLabel(title, position: position)
            case let (alignment?, nil):
                content.chartYAxisLabel(title, position: position, alignment: alignment)
            case let (nil, spacing?):
                content.chartYAxisLabel(title, position: position, spacing: spacing)
            ...
            switch (alignment, spacing) {
            case (nil, nil):
                content.chartYAxisLabel(position: position) {
                    context.buildChildren(of: element, forTemplate: template)
                }
            case let (alignment?, nil):
                content.chartYAxisLabel(position: position, alignment: alignment) {
                    context.buildChildren(of: element, forTemplate: template)
                }
            case let (nil, spacing?):
                content.chartYAxisLabel(position: position, spacing: spacing) {
                    context.buildChildren(of: element, forTemplate: template)
                }
            case let (alignment?, spacing?):
